...
| 10 | Baselines for seven discs, side by side |
| 11 | The order of work that worked |
| 12 | The seven `sha1-all.txt` lists, and what a file-level hash cannot see |

Findings confirmed on every disc so far are marked **[all]**; those confirmed on
fewer are marked **[N of 7]**. Every mark now reads `of 7`: the seventh pipeline
...
The seven bracket the format: one disc where the program owns its assets, one
...
idiom the others use, and one from a year before *that* which is not a game —
four rooms that sit still until a three-year-old points at something.

## The result that made this repository worth splitting out

Sectors in front of the file system belong to no file, and on all seven discs
...
outright six times, and it is exactly the kind of thing that has to live in one
...
generations of two discs each**. 714,240 bytes of the 1993 one — RL7 video,
...
And **2,246,756 bytes of the 1991–92 one — eight streams, CLUT4 video on two
...
publishes `notes/sha1-all.txt`** — **338 records over seven discs**, generated
for all seven at once. Comparing them finds two crossings, one of which is a
ten-byte path table on four discs, while **7,475,756 bytes are shared across
discs and invisible to every one of the 338 records**. On this platform the unit of sharing is the *stream* and the
...
2. Mark it **[all]** or **[N of 7]** only if you have actually checked the other
